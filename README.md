...
* [`src/fetchimagefsm4/`](src/fetchimagefsm4/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/fetchimagefsm4/core.cljs): contains the SPA entry point, `init`
* [`test/fetchimagefsm4/`](test/fetchimagefsm4/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `fetchimagefsm4` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fetchimagefsm4/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fetchimagefsm4.example
  (:require [fetchimagefsm4.config :as config])
